...
class ProfileController extends BaseController
{
    public function show()
    {
        try {
            ...
            // Get user data safely without using deprecated properties
            ...
            try {
                ...
                Log::info('Stats calculated successfully', ['user_id' => $user->id, 'stats' => $stats]);
            } catch (\Exception $e) {
                Log::warning('Stats calculation failed, using defaults', [
                    'user_id' => $user->id,
                    'error' => $e->getMessage()
                ]);
                
                // Provide safe default stats
                $userData['stats'] = [
                    'total_workouts' => 0,
                    'total_minutes' => 0,
                    'total_calories' => 0,
            ...
            try {
                ...
                Log::info('BMI calculated successfully', ['user_id' => $user->id, 'bmi' => $bmiInfo]);
            } catch (\Exception $e) {
                Log::warning('BMI calculation failed, using defaults', [
                    'user_id' => $user->id,
                    'error' => $e->getMessage()
                ]);
            ...
            Log::info('Profile data prepared successfully', [
                'user_id' => $user->id,
                'has_stats' => isset($userData['stats']),
                'profile_completion' => $userData['stats']['profile_completion'] ?? 0
            ]);
    ...
    public function update(Request $request)
    {
        try {
            ...
            foreach ($validated as $key => $value) {
                ...
                } elseif (in_array($key, ['age', 'height', 'weight', 'gender', 'location', 'bio', 'blood_group', 'phone', 'date_of_birth'])) {
    ...
    /**
     * Get user dashboard data (for backward compatibility)
     */
    public function dashboard()
    {
        try {
            ...
            $dashboardData = [
                'user' => $user->toArray(),
                'stats' => $user->getStats(),
                'recent_workouts' => $user->workoutSessions()
                    ->where('status', 'completed')
                    ->latest('completed_at')
                    ->limit(5)
                    ->get(),
                'active_goals' => $user->goals()
                    ->where('status', 'active')
                    ->limit(3)
                    ->get(),
            ];
            ...
            return $this->handleException($e, 'Dashboard data loading');
        }
    }
}