...
class StreakCalculatorService
{
    ...
    private function calculateMonthlyConsistency(User $user): array
    {
        ...
        for ($i = 0; $i < 12; $i++) {
            ...
            $workoutsThisMonth = $user->workoutSessions()
                                    ->where('status', 'completed')
    ...
    public function getUpcomingMilestones(User $user): array
    {
        ...
        $currentStreak = $this->calculateWorkoutStreak($user);
        $totalCalories = $user->workoutSessions()->where('status', 'completed')->sum('calories_burned');
        $totalHours = $user->workoutSessions()->where('status', 'completed')->sum('duration_minutes') / 60;
